...
Phase 2 of our project is coming to a close, as well as the project itself. Although the initial plan was to continue to phase 3 with performance optimizations and GPU rendering, we got a bit tired of looking at the torus and decided to start a spin-off project with something more interesting to look at. In this post, we're going to look back at our progress since the end of [Phase 1][post-from-pixel-to-torus].

[Version 0.1.9 on GitHub][version-0-1-9]{: .no-github-icon}

## What you will see today

As you remember, our goal for Phase 2 was inspired by the [three.js Geometry Browser][geometry-browser], and it looks like by and large we've achieved that goal. Our current scene is still a tumbling torus under a fixed orthographic camera, but the look has changed quite a bit since the [Phase 1 capstone][post-from-pixel-to-torus].
...
We now have more flexible material settings and multiple light sources of two distinct types: directional and point lights.
...
As usual, we moved forward in rather small increments: each release added one idea and proved it in a visible artifact. Here is the progression at a glance:
...
[Three Lights Are Better Than One][post-three-lights] added multiple-light setups to the renderer's capabilities. Three directional lights, adapted from the Geometry Browser into our left-handed coordinates, softened the shading and made the highlights more interesting.

[Introducing Point Lights][post-point-lights] went further: we've introduced another type of light source. That forced a deeper change in the rasterizer — interpolate world-space position alongside the normal, bundled as a [`SurfacePoint`][source-surface-point] — and combined both light types into a unified [`Light`][source-light] struct.

### Shaders without a GPU

By the time we finished with the point lights, we noticed that we had been re-implementing the same concept of linear interpolation in triangles three times: interpolate vertex colors (Gouraud), interpolate normals (Phong), then interpolate normals _and_ world positions. [Shaders: Generalize Render Pipeline][post-shaders] split that pattern into a fixed rasterizer plus a [`Shader`][source-shader] trait — vertex shading produces interpolatable data; pixel shading turns the interpolated value into a color.

Basically, we've arrived at the concept of shaders: the way modern GPU-backed render pipelines work. Although the implementation was rather rudimentary, it was a breakthrough moment that helped us better understand the design principles of rendering pipelines.

The satisfying proof of the concept was putting [_Gouraud_][gouraud] and [_Phong_][phong] back side by side as plug-in shaders on the same pipeline. For demos, we still prefer Phong; Gouraud remains as a teaching tool and a check that the abstraction is real.
...
| **Scene look** | Geometry-browser palette, gray clear color, mixed directional and point lights |
...
The topic of computer graphics is limitless, and we've only scratched the surface with this project. It was a lot of fun to implement the rendering pipeline from scratch, but at the end we got bored of looking at the same torus again and again. It served its purpose for now: we need to come up with a fresh idea for a next project to continue exploring the fascinating field of computer graphics.
