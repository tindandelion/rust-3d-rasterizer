...
We sample the surface with two angles, each sweeping a full turn $t \in [0, 2\pi)$:
...
Each grid cell becomes two triangles, exactly like a latitude-longitude patch on a sphere but without poles that pinch. The export bins use **`torus(48, 32)`** — 1536 vertices and 3072 facets — which is dense enough for a smooth silhouette at 800×600.
