...
Wireframe drawing walks [`edges()`][source-edges], which yields the twelve segments **after** the current transform. Each export path picks a matrix, hands the cube to [`draw_edges`][source-draw-edges], and the line rasterizer does not care which pose we chose.
...
Until now [`WebpEncoder`][source-webp-encoder] accepted a single framebuffer and wrote one still image. [WebP][webp] is not only a still image codec: the same container can hold an **[animated image][animated-webp]** — a sequence of frames with per-frame timing, like a compact GIF. We picked lossless animated WebP over GIF to keep the same RGB encode path we use for stills, built with the [`webp-animation`][webp-animation] crate.
...
Playback speed is not implicit in “360 frames”; the animated WebP format stores an explicit **timestamp in milliseconds** on each frame. [`WebpEncoder::with_frame_spacing`][source-frame-spacing] wraps `webp_animation::Encoder`: on each [`add_frame`][source-add-frame] we pass the current timestamp, then advance it by [`FRAME_SPACING_MS`][source-frame-spacing-ms] (**20 ms**). That is **1000 / 20 → 50 fps**.
