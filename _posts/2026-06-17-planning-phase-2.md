...
[Phase 1][post-from-pixel-to-torus] laid the basic groundwork of the 3D rasterizer pipeline. After [recording baseline performance benchmarks][post-baseline-benchmarks], we're ready to make a plan for the next phase. By our intention, we're not going to introduce major new features during phase 2. Instead, we'd like to focus on the concepts we've already implemented, revisit them and deepen our understanding of certain subjects. 
...
In phase 2, we don't plan to introduce any big new features, such as textures or shadows. Instead, we'd like to use this phase as a driver to explore certain topics in more depth. If phase 1 was like building a walking skeleton, in this phase we'll add more meat to it.
...
We'll start from the most visual part: borrowing the color palette from `three.js` and extending our `Material` data type and shading equations to match. We documented that first step in [Materials, Colors, and the Stage][post-materials-colors-stage].
