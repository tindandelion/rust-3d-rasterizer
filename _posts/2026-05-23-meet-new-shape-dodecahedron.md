...
[The previous version][post-cube-gets-light] gave us a shaded cube and diffuse reflection. However, our code was tightly coupled to that cube implementation, which bothered Sergey, who wanted the code to be more generic. So he decided to add an intermediate milestone and have a bit more fun with faceted objects. We decided to introduce one more shape into our codebase: a [dodecahedron][dodecahedron-wikipedia]. That led to a few interesting changes and generalizations in the rendering pipeline. 

[Version 0.0.9 on GitHub][version-0-0-9]{: .no-github-icon}

## What you will see

The shape we're rendering is no longer the cube. Instead, you will see a rotating shaded dodecahedron: a more interesting shape with more detail. 
...
Until now, we've been [focusing only on the cube shape][post-cube-paints-six-faces], and that impacted the code: our drawing primitives were **quads** — 4-vertex convex polygons in 2D space. That worked well for the cube, but was not well suited to the dodecahedron. 

As you can see, the dodecahedron has 5-vertex facets: something you can't efficiently represent with quads. Introducing a special 5-vertex 2D polygon didn't feel like a scalable way to go, either. Technically, it's possible to have an n-vertex convex polygon as a drawing primitive, but graphics engines tend to go in the opposite direction: instead of inventing more and more complex shapes, we find ways to represent them with simpler, more generic primitives. And what's the simplest primitive for a piece of a flat surface? A triangle. 
...
Let's have a look at the major changes that we made in the code. This version did not introduce brand new algorithms or concepts. It was mostly about generalizing our existing code to make it more versatile. As the main artifacts of this milestone, we now have: 
...
So we have a pretty generic `Shape` data type now. We also have a couple of constructor functions that return `Shape` instances for our pre-defined shapes: a cube and a dodecahedron.

[`unit_cube`][source-unit-cube] shape sits in the same **[-½, ½]³** box as before, but each of the six quads is now two facets — a wedge split `(w, x, y)` and `(w, y, z)` with consistent counter-clockwise winding when seen from outside along the face normal. Eight vertices, twelve facets, no separate `Quad` type anymore.

[`unit_dodecahedron`][source-unit-dodecahedron] builds a dodecahedron shape from the golden-ratio vertex table and thirty-six facets. Cursor took the data from [three.js `DodecahedronGeometry`][threejs-dodecahedron] at detail _0_ — three triangles per pentagon, precomputed index list. Each facet’s normal is derived from its corner positions so lighting stays consistent after transforms.
