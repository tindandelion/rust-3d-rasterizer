...
One common way to represent a sphere as a triangular mesh is a [_UV (lat-lon) sphere_][uv-sphere]. Essentially, we divide the sphere with a latitude/longitude grid and calculate a vertex coordinate for each intersection point. The downside of this approach, though, is that the resulting rectangles differ greatly in shape: they get thinner as we move closer to the poles.
...
### Octasphere: a simpler icosphere-style start
...
This flamegraph shows that we spend most of the time in two parts of the program:

* producing the WebP image takes about 66% of the overall time;
* the rest is spent in the `FillTriangle` drawing primitive.
