...
- **Geometry API:** **`scene::shape::Shape`** (**`Vec<Vec3>`** + **`Vec<Facet>`** behind **`vertices()`** / **`facets()`** **`&`** slice accessors) implements **`TriMesh`**; procedural **`scene::cube::unit_cube()`** and **`scene::dodecahedron::unit_dodecahedron()`** return **`[-½, ½]³`‑boxed **`TriMesh`** shapes** (Plato dodeca: same axis bounds as **`unit_cube`**, not Plato “edge = 1” circumradius sizing). **`visible_facets`** yields **`Triangle`** (corners **`[Vec3; 3]`**, facet **`UnitVec3`**) per front **`Facet`**; **`draw_facets`** uses one **`FillTriangle`** draw per **`Triangle`**. **`Sphere`/torus** add geometry on this stack only; evolve **`Vertex`** only when a milestone needs new attributes.
