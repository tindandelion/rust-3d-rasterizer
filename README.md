...
**Thorus Forge** is a personal learning project: a 3D software rasterizer in Rust, developed mainly on macOS. The plan is to build the math and rasterization path on the CPU first, then add optional GPU rendering with wgpu (Metal on Mac). Scenes stay small and procedural—working up shapes like a cube, sphere, and eventually a torus — with lossless WebP still images and animations as the main artifacts.
