...
//! **`NEG_Z`**). **`BlinnShadingModel`** toward **`NEG_Z`** with a high-ambient matte yields uniform corner intensity, so
...
use glam::{UVec2, Vec3};
use thorus_forge::{
    BlinnShadingModel, Camera, FrameBuffer, Material, SHAPE_BASE_COLOR, draw_facets,
...
fn draw_unit_cube() {
    ...
    let light = BlinnShadingModel::new(-camera.direction(), Material::matte(0.9));
    let mesh = cube();
