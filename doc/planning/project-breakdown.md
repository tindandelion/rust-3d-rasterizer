...
### [ ] Materials & lights — decouple and explicit Phong colors

- **Goal:** Split **`BlinnLightModel`** into scene **lights** and per-shape **`Material`**. **`Material`** on **`Shape`**: **`diffuse`**, **`emissive`**, **`specular`** (**`Rgb`**) + **`shininess`** (**`i32`**) — retire factor-based **`Material::matte`** / **`Material::shiny`**. **`Light`**: **`direction`** + **`intensity`** (**white only**; colored lights deferred). Refactor **`Shape::render`** to accept **`&[Light]`** and sum per-light diffuse/specular; add **emissive** once per fragment. **`FrameBuffer::clear(Rgb)`** for explicit scene background. **No `Scene` struct yet** — export bins wire lights + shapes directly until multi-light wiring earns a container.
- **Outcome:** **`still-scene`** / **`animated-scene`** use the **Three.js reference palette** with **one** white directional light and dark-gray clear. Unit tests cover decoupled shading math with small fixtures.

### [ ] Multi-light — summed directional contributions

- **Goal:** After single-light decoupling works, add **2–3** white **`Light`** sources (direction + intensity) whose contributions **sum** per fragment — approximating the geometry browser’s three **`DirectionalLight`** setup. Introduce **`Scene`** only if bin wiring becomes painful.
...
- **Phase 1 order:** **`Camera: arbitrary eye`** **[x]** … **`Torus: filled + smooth shading`** **[x]** (**`meshes::torus`**, export bins). **Phase 1 complete** (orthographic CPU stack). **Next — Phase 2:** **`Materials & lights`** (decouple, explicit colors, clear color, Three.js palette in bins) → **`Multi-light`** → optional **`Perspective projection (CPU)`** stretch. **Phase 3:** **`wgpu`**. **Out of Phase 2:** wireframe overlay, flat shading, extended torus API, live **`winit`** viewer. **Overall rhythm:** geometry + Phong + depth on CPU first; Phase 2 enriches **materials/lights/colors** export-first.
