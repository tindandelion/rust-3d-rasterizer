...
- **Shipped:** **`meshes::torus(ring_segments, tube_segments)`** (**`src/meshes/torus.rs`**) — major circle in **XZ**, tube around **Y**, default radii **`0.5`** / **`0.22`**, parametric smooth normals; **`still-scene`** (**`torus(24, 16)`** + ground plane); **`animated-scene`** (**orbiting camera**, same **Phong** path).
...
- **Animated export:** **`animated-scene`** (**`src/bin/animated-scene.rs`**) — **lossless animated WebP**: **`meshes::torus(24, 16)`** at the origin, **Phong** **`BlinnLightModel`**, **`360`** eased camera-orbit frames at **`ANIMATED_SCENE_FRAME_SPACING_MS`** (default output **`scene.webp`**, overridable via argv). **`still-scene`** (**`src/bin/still-scene.rs`**) — **Phong** **`meshes::torus(24, 16)`** still (**`Material::shiny(0.15, 100.0)`**) over a **terracotta ground plane**. Cube regression: **`tests/draw_unit_cube.rs`** (no **`still-cube`** bin).
