...
- **Outcome:** Reliable **overlap resolution** when **draw order stops being safe**—ready for **filled torus** (and similar self-occluding opaque meshes). **Perspective depth migration** stays in **`Perspective projection`** (**after torus**).

### [x] Torus: filled + smooth shading

- **Goal:** Land the **long-term visual target** on the **unified filled triangle** stack—**skip torus wireframe** (**see Notes / deferred**): **depth buffer**, **Phong**, and **self-occluding** meshes are already exercised on **sphere**; go straight to a **procedural torus** (**`meshes::torus`** or equivalent) as an indexed **`Shape`** with **smooth vertex normals** (parametric **\( \hat{n} \)** on the tube surface). Reuse **`draw_facets`** / **`PhongShadedTriangle`** (**interpolated normals + Blinn–Phong specular** from **`Sphere: Phong shading`**) and **`write_pixel`** depth for **tube/hole overlap**. **Orthographic camera** retained through this milestone—the **projection matrix** swaps in **`Perspective projection`**, **next**.
