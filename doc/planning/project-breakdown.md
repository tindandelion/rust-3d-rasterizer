...
- **Shipped:** Indexed **`Cube::visible_faces`** **`(slot, Quad)`**, **`FillQuad`** (bbox + half-plane inside test), **`draw_faces`** + **`CUBE_FACE_PALETTE`**; **`still-cube`** / **`animated-cube`** call **`draw_faces`** (filled visual regression is covered by in-crate tests such as **`tests/draw_unit_cube.rs`** rather than a committed WebP golden).
