...
- **Current module layout:** **`geometry`** — **`Mesh`**, **`Facet`**, **`Triangle`**, **`UnitVec3`** (re-exported; private **`geometry/{mesh,facet,unit_vec3}.rs`**). **`meshes`** — **`cube()`**, **`dodecahedron()`**, **`sphere(splits)`** (re-exported; private **`meshes/{cube,dodecahedron,sphere}.rs`**). Scene-level **`Shape`** (**`mesh` + `color`**, **`Shape::render`**), scene constants — **`src/lib.rs`**. Export bins — **`src/bin/`** (**`still-scene`**, **`animated-scene`**).
- **Animated export:** **`animated-scene`** (**`src/bin/animated-scene.rs`**) — **lossless animated WebP**: two **`meshes::sphere(4)`** instances (**radius 0.3** at **`(±0.5, 0, 0)`**, distinct colors), **Phong** **`BlinnLightModel`**, **`360`** eased camera-orbit frames at **`ANIMATED_SCENE_FRAME_SPACING_MS`** (default output **`scene.webp`**, overridable via argv). **`still-scene`** (**`src/bin/still-scene.rs`**) — **Phong** **`meshes::sphere(4)`** still (**`0.7`** uniform scale, **`Material::shiny(0.15, 100.0)`**) over a **terracotta ground plane**. Cube regression: **`tests/draw_unit_cube.rs`** (no **`still-cube`** bin).
- **Iteration order:** **`Camera: arbitrary eye`** **[x]** is **orthogonal** to **orthographic vs perspective**: **orbit-style **`look-at`** (eye anywhere sensible, target **scene center** defaulting to **world origin**, **world +Y up**) is **landed** in **`ortho_camera`**. **`Sphere: triangular mesh`** **[x]** (**`meshes::sphere`**). **`Sphere: Gouraud shading`** **[x]** (**`GouraudShadedTriangle`**, radial vertex normals). **`Sphere: Phong shading`** **[x]** (**`PhongShadedTriangle`**, **`Shape::render`**). **`Depth buffer`** **[x]** (**view-space **`z_\text{view}`**, **`write_pixel`** depth test, **`PhongShadedTriangle`** **`z`** lerp). **Next:** **torus** milestones under the same **orthographic** stack. **`Perspective projection`** follows **filled torus** and **inherits** that **`look-at`** policy (**projection** swaps to **`w`** divide afterward); **NDC / inverse depth** and **`wgpu`** compare/range alignment **deferred** there / Phase 2. **Overall rhythm:** **cube + dodecahedron + sphere (triangle **`Mesh`**) → Gouraud → Phong → depth → torus** stays **orthographic** first (simpler **\(w\)**-free correctness). **`BlinnLightModel`** **`toward_eye`**: **constant **`−Camera::direction()`** today (ortho); **per-point **`normalize(eye − position)`** when **perspective** lands (**see that milestone**).
