...
- **Goal:** Refactor **`Material`** onto **`Shape`** and decouple it from the light source. **`Material`**: **`diffuse`**, **`emissive`**, **`specular`** (**`Rgb`**) + **`shininess`** (**`i32`**) — retire factor-based **`Material::matte`** / **`Material::shiny`** and the bundled **`BlinnLightModel`**. Introduce **`Light`**: **`direction`** + **`intensity`** (**white only**; colored lights deferred). Refactor **`Shape::render`** to take **`&Light`** (single source) and evaluate diffuse/specular from explicit material colors; add **emissive** once per fragment. **`FrameBuffer::clear(Rgb)`** for explicit scene background. **No `Scene` struct yet** — export bins wire one light + shape directly. **Defer multi-light summation** to the next milestone (**do not** take **`&[Light]`** here).
