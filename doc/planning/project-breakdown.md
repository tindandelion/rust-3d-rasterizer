...
- **Shipped:** Indexed **`Cube::visible_faces`** **`(slot, Quad)`**, **`FillQuad`** (bbox + half-plane inside test), **`draw_faces`** + **`CUBE_FACE_PALETTE`**; **`still-cube`** / **`animated-cube`** call **`draw_faces`**; golden **`snapshots/cube/scene.webp`** refreshed for filled output.
