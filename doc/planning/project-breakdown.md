...
- **Goal:** **Switch the whole filled pipeline to triangles** (**one** **`[Vertex; 3]`** stream **+** **one** filled **triangle** raster implementation — **half-space / barycentric vs scanlines**, pick once and reuse afterward). **Refactor `scene/cube`** to **`[Vertex; 3]`** (**two triangles per quad face**, **consistent winding** with the new raster), **delete** **`FillQuad` / quad-filled submit**, and reuse the **triangle path** for **existing** **`still-cube`** / **`animated-cube`** exports (**`DiffuseLight`**, orthographic tumble, …). **Ship** the **same stack** on a **regular dodecahedron** (**twelve pentagonal faces**, each triangulated—e.g. **centroid fan** → **five** triangles per pentagon, **60** total—**planned winding**) as the **demonstrator** (**nicer silhouette** than a cube refactor alone, **and** **pentagons force** triangles—they cannot reuse the interim **quad** inner test).
...
- **Iteration order:** **`Perspective projection`** deliberately follows **filled torus** so **cube (quad interim) → dodecahedron (triangles everywhere) → sphere → depth → torus** stays entirely **orthographic** first (simpler **\(w\)**-free correctness). **`Cube: quad stream — filled faceted raster`** (**shipped**) was the **temporary** **`FillQuad`** path—**removed** when **`Dodecahedron: triangular mesh`** **refactors the cube**, **drops** quad fill, **and adds** the **triangle** raster + **dodecahedron**. **`Sphere`** **only adds** tessellated sphere geometry onto that stack. **`Depth buffer`** sits **before torus** for **tube/hole overlap** under **orthographic** **`z`**; revisit **\(z_{\text{NDC}}\)** / depth behavior once **perspective** lands (**see `Perspective projection`**). The **shipped quad** milestones still skipped **`z`** for a sane **single convex exterior** cube reading.
