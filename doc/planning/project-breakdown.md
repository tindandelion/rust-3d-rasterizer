...
- **Outcome:** **`still-scene`** / **`animated-scene`** use the palette above (diffuse **`0x156289`**, emissive **`0x072534`**, specular **`0x111111`**, **`shininess` 30**, clear **`0x444444`**) with **one** white directional light at **`intensity` 1.0**; unit tests cover explicit material colors, single-light Phong math, and **`with_intensity`** scaling.
...
- **Current module layout:** **`geometry`** — **`Mesh`**, **`Facet`**, **`Triangle`**, **`UnitVec3`** (re-exported; private **`geometry/{mesh,facet,unit_vec3}.rs`**). **`meshes`** — **`cube()`**, **`dodecahedron()`**, **`sphere(splits)`**, **`torus(ring_segments, tube_segments)`** (re-exported; private **`meshes/{cube,dodecahedron,sphere,torus}.rs`**). Scene-level **`Shape`** (**`mesh` + `color`**, **`Shape::render`**), scene constants — **`src/lib.rs`**. Export bins — **`src/bin/`** (**`still-scene`**, **`animated-scene`**).
- **Export bins:** **`animated-scene`** (**`src/bin/animated-scene.rs`**) — **lossless animated WebP** at **`SCENE_WIDTH` × `SCENE_HEIGHT`** (**800×600**): **`meshes::torus(48, 32)`** at the origin with **world-fixed **`R_z R_y R_x`** tumble** (**`α = β = γ = t`**, seamless loop), **fixed camera** (eye **`(0, 0.5, −1)`**), **Phong** **`DirectionalLight::new`** (**`intensity` 1.0**), **`ANIMATED_SCENE_FRAME_COUNT`** frames at **`ANIMATED_SCENE_FRAME_SPACING_MS`** (default output **`scene.webp`**, overridable via argv). **`still-scene`** (**`src/bin/still-scene/`** — **`main.rs`** + private **`kitty_terminal.rs`**) — same **Phong** **`meshes::torus(48, 32)`**, fixed camera, geometry-browser **`Material`**, **`DirectionalLight::new`** (**`intensity` 1.0**); framebuffer size follows **terminal pixel dimensions** (**`crossterm`** window size, with a cell-count fallback when pixels are unavailable). **Primary presentation:** centered **24-bit RGB** image via the **Kitty graphics protocol** (alternate screen, raw mode, any-key dismiss); **in parallel** (background thread) writes **`still-scene.webp`** at the same resolution (lossless). Cube regression: **`tests/draw_unit_cube.rs`** (no **`still-cube`** bin). **Lighting notes:** **`doc/notes-on-lighting.md`** (byte-space vs Three.js linear, **`intensity` policy**).
- **Phase 1 order:** **`Camera: arbitrary eye`** **[x]** … **`Torus: filled + smooth shading`** **[x]** (**`meshes::torus`**, export bins). **Phase 1 complete** (orthographic CPU stack). **Next — Phase 2:** **`Material — explicit Phong colors (single light)`** → **`Multi-light`** → optional stretch **`Perspective projection (CPU)`** → optional stretch **`Positional lights`**. **Phase 3:** **`wgpu`**. **Out of Phase 2:** wireframe overlay, flat shading, extended torus API, live **`winit`** viewer. **Overall rhythm:** geometry + Phong + depth on CPU first; Phase 2 enriches **materials/lights/colors** export-first.
