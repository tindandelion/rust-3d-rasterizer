...
- **Outcome:** Reliable **overlap resolution** when **draw order stops being safe**—the **immediate consumer** is **filled torus** (and similar self-occluding opaque meshes). Optionally **stress on cube** (**two overlapping cubes**) if you want a simpler regression harness than the torus alone; **perspective depth migration** stays in **`Perspective projection`** (**after torus**).
...
- **Animated export:** **`animated-scene`** (**`src/bin/animated-scene.rs`**) — **lossless animated WebP**: two **`meshes::sphere(4)`** instances (**radius 0.3** at **`(±0.5, 0, 0)`**, distinct colors), **Phong** **`BlinnLightModel`**, **`360`** eased camera-orbit frames at **`ANIMATED_SCENE_FRAME_SPACING_MS`** (default output **`scene.webp`**, overridable via argv). **`still-scene`** (**`src/bin/still-scene.rs`**) — **Phong** **`meshes::sphere(4)`** still (**`0.7`** uniform scale, **`Material::shiny(0.15, 100.0)`**). Cube regression: **`tests/draw_unit_cube.rs`** (no **`still-cube`** bin).
- **Iteration order:** **`Camera: arbitrary eye`** **[x]** is **orthogonal** to **orthographic vs perspective**: **orbit-style **`look-at`** (eye anywhere sensible, target **scene center** defaulting to **world origin**, **world +Y up**) is **landed** in **`ortho_camera`**. **`Sphere: triangular mesh`** **[x]** (**`meshes::sphere`**). **`Sphere: Gouraud shading`** **[x]** (**`GouraudShadedTriangle`**, radial vertex normals). **`Sphere: Phong shading`** **[x]** (**`PhongShadedTriangle`**, **`Shape::render`**). **`Depth buffer`** remains open (**depth → torus**); **chosen rule:** **view-space **`z_\text{view}`** (orthographic only)** — **NDC / inverse depth** and **`wgpu`** compare/range alignment **deferred** to **`Perspective projection`** / Phase 2. **`Perspective projection`** still follows **filled torus** and **inherits** that **`look-at`** policy (**projection** swaps to **`w`** divide afterward). **Overall rhythm:** **cube + dodecahedron + sphere (triangle **`Mesh`**) → Gouraud → Phong → depth → torus** stays **orthographic** first (simpler **\(w\)**-free correctness). **`Depth buffer`** sits **before torus** for **tube/hole overlap** under **view-space **`z`**. **`BlinnLightModel`** **`toward_eye`**: **constant **`−Camera::direction()`** today (ortho); **per-point **`normalize(eye − position)`** when **perspective** lands (**see that milestone**).
