...
- **Outcome:** **Phong-shaded filled torus** under **orthographic projection** (**Phase 1 CPU rasterizer capstone**); **Phase 2** follows (**perspective projection**, then **GPU / wgpu**). **Still** and/or **animated** **`.webp`** exports as appetite allows.

## Phase 2

### [ ] Perspective projection (CPU)

- **Goal:** After the **Phase 1** orthographic stack is proven, swap the **camera / projection block** to **perspective**: **homogeneous coordinates**, **`w`** divide, and **explicit guardrails** for **near/far**, **\(w \le 0\)**, and **behind-camera** geometry (**`wgpu` parity** on depth range / NDC handedness stays a **checkpoint for the GPU / wgpu milestone** unless you tighten it earlier). Keep the **`Camera: arbitrary eye`** convention (**eye position**, **scene-center target**, **world +Y up**) for **`look-at`**; perspective only replaces the **projection** half of the matrices. **Revisit depth buffer:** **`Depth buffer`** ships **view-space **`z_\text{view}`** for orthographic overlap** — under perspective, **re-evaluate** stored depth (**likely **`z_{\text{NDC}}`** or perspective-correct **`z/w`**) and **interpolation** before trusting overlap on self-occluding meshes. **Lighting:** under orthographic projection, **`Shape::render`** passes a **constant** **`toward_eye`** derived from **`Camera::direction()`** (parallel sight lines); **perspective** requires revisiting this — **per-fragment** **`normalize(eye − surface_point)`** (pinhole / **toward-eye** varies with position) so specular and the projection model stay aligned.
- **Outcome:** **`WebP`** still + animated demos that **replay** representative scenes (**cube** wireframe and/or fills, **Phong torus** — as much as appetite allows) **only changing** projection (**same tumble / timing policies** wherever you mirror prior exports). Bridges the **Phase 1** orthography stack to **`wgpu`/NDC intuition** ahead of **GPU / wgpu**.

### [ ] GPU / wgpu (to elaborate)
...
- **Phase 1 order:** **`Camera: arbitrary eye`** **[x]** is **orthogonal** to **orthographic vs perspective**: **orbit-style **`look-at`** (eye anywhere sensible, target **scene center** defaulting to **world origin**, **world +Y up**) is **landed** in **`ortho_camera`**. **`Sphere: triangular mesh`** **[x]** (**`meshes::sphere`**). **`Sphere: Gouraud shading`** **[x]** (**`GouraudShadedTriangle`**, radial vertex normals). **`Sphere: Phong shading`** **[x]** (**`PhongShadedTriangle`**, **`Shape::render`**). **`Depth buffer`** **[x]** (**view-space **`z_\text{view}`**, **`write_pixel`** depth test, **`PhongShadedTriangle`** **`z`** lerp). **`Torus: filled + smooth shading`** **[x]** (**`meshes::torus`**, export bins). **Phase 1 complete** (orthographic CPU stack). **Next — Phase 2:** **`Perspective projection (CPU)`** under the same **`look-at`** policy (**projection** swaps to **`w`** divide; depth + lighting revisit); then **`GPU / wgpu`**. **Overall rhythm:** **cube + dodecahedron + sphere (triangle **`Mesh`**) → Gouraud → Phong → depth → filled torus** stayed **orthographic** first (simpler **\(w\)**-free correctness). **`BlinnLightModel`** **`toward_eye`**: **constant **`−Camera::direction()`** today (ortho); **per-point **`normalize(eye − position)`** when **perspective** lands (**see **`Perspective projection (CPU)`**).
...
- **Live presentation:** **`still-scene`** landed **Kitty-terminal** display (see **Export bins** above). **`winit`** window + real-time **animated** playback remains optional—pairs naturally with **`animated-scene`** timing instead of writing WebPs.
