...
- **Goal:** If appetite remains after materials/lights: swap **projection** to **perspective** (**`w`** divide, near/far guardrails, depth model migration, per-fragment **`toward_eye`**) while keeping **`look-at`** and export-first workflow. **Not** required for Phase 2 completion; **not** a Phase 3 prerequisite.
- **Outcome:** Representative **WebP** exports replay torus scenes with perspective only where mirrored exports exist today.

### [ ] Positional lights — optional stretch

- **Goal:** If appetite remains after perspective (or after multi-light if perspective is skipped): extend the light model beyond **directional** sources. Introduce **positional** lights (**e.g.** point lights with **`position`** + **`intensity`**, white only for now) where **`toward_light`** is **`normalize(light_position − surface_point)`** per fragment (perspective-correct when **`surface_point`** is in world or view space consistently). Sum with existing directional **`Light`** contributions in **`Shape::render(&[Light])`**. Optional distance attenuation only if needed for a readable export — keep the first pass simple.
- **Outcome:** Export bin or test scene demonstrates **directional + positional** lighting on the torus under the Phase 2 material palette; orthographic exports remain valid if perspective stretch was skipped.

### [ ] Lighting parity — Three.js equation alignment (tentative)

- **Goal:** Revisit **`Material::shade`**, **`DirectionalLight`** contrib terms, and export-bin **`default_material()`** so renders match the geometry-browser **`MeshPhongMaterial`** palette without ad-hoc specular/shininess tweaks. **Browser reference:** diffuse **`0x156289`**, emissive **`0x072534`**, specular **`0x111111`**, **`shininess` 30**. **Today:** **`default_material()`** uses specular **`0x444444`** and **`shininess` 100** as a temporary visual compromise. Compare output against **`threejs/index.html`** and the [official TorusGeometry browser](https://threejs.org/docs/scenes/geometry-browser.html#TorusGeometry); document residual gaps in **`doc/notes-on-lighting.md`**. **Last Phase 2 step** — run after **Multi-light** and optional stretches when export wiring is stable.
- **Outcome:** Export bins adopt browser specular **`0x111111`** and **`shininess` 30** (or documented, tested acceptable delta); golden **`still-scene.webp`** refreshed; eyeball parity with Three.js improved vs today’s tweaked material.
...
- **Current module layout:** **`geometry`** — **`Mesh`**, **`Facet`**, **`Triangle`**, **`UnitVec3`** (re-exported; private **`geometry/{mesh,facet,unit_vec3}.rs`**). **`meshes`** — **`cube()`**, **`dodecahedron()`**, **`sphere(splits)`**, **`torus(ring_segments, tube_segments)`** (re-exported; private **`meshes/{cube,dodecahedron,sphere,torus}.rs`**). **`lighting`** — **`Material`**, **`DirectionalLight`**, linear **`Color`** (**`src/lighting/color.rs`**). Scene-level **`Shape`** (**`mesh` + `material`**, **`Shape::render`**), scene constants — **`src/lib.rs`**. Export bins — **`src/bin/`** (**`still-scene`**, **`animated-scene`**).
- **Export bins:** **`animated-scene`** (**`src/bin/animated-scene.rs`**) — **lossless animated WebP** at **`SCENE_WIDTH` × `SCENE_HEIGHT`** (**800×600**): **`meshes::torus(48, 32)`** at the origin with **world-fixed **`R_z R_y R_x`** tumble** (**`α = β = γ = t`**, seamless loop), **fixed camera** (eye **`(0, 0.5, −1)`**), **`default_material()`**, **`SCENE_BACKGROUND`** clear, **`DirectionalLight::new`** (**`intensity` 1.0**), **`ANIMATED_SCENE_FRAME_COUNT`** frames at **`ANIMATED_SCENE_FRAME_SPACING_MS`** (default output **`scene.webp`**, overridable via argv). **`still-scene`** (**`src/bin/still-scene/`** — **`main.rs`** + private **`kitty_terminal.rs`**) — same **`meshes::torus(48, 32)`**, fixed camera, **`default_material()`**, **`SCENE_BACKGROUND`**, **`DirectionalLight::with_intensity(..., 1.0)`**; framebuffer size follows **terminal pixel dimensions** (**`crossterm`** window size, with a cell-count fallback when pixels are unavailable). **Primary presentation:** centered **24-bit RGB** image via the **Kitty graphics protocol** (alternate screen, raw mode, any-key dismiss); **in parallel** (background thread) writes **`still-scene.webp`** at the same resolution (lossless). **Golden WebP:** **`test-data/still-scene.webp`** compared in **`still-scene`** bin test. Cube regression: **`tests/draw_unit_cube.rs`** (no **`still-cube`** bin). **Lighting notes:** **`doc/notes-on-lighting.md`** (linear color path, **`intensity` policy**).
- **Phase 1 order:** **`Camera: arbitrary eye`** **[x]** … **`Torus: filled + smooth shading`** **[x]** (**`meshes::torus`**, export bins). **Phase 1 complete** (orthographic CPU stack). **Phase 2 — shipped:** **`Material — explicit Phong colors (single light)`** **[x]**. **Next — Phase 2:** **`Multi-light`** → optional stretch **`Perspective projection (CPU)`** → optional stretch **`Positional lights`** → **`Lighting parity — Three.js equation alignment (tentative)`**. **Phase 3:** **`wgpu`**. **Out of Phase 2:** wireframe overlay, flat shading, extended torus API, live **`winit`** viewer. **Overall rhythm:** geometry + Phong + depth on CPU first; Phase 2 enriches **materials/lights/colors** export-first.
