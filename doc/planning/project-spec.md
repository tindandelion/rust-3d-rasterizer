...
- **Phase 2 gaps (vs [Three.js TorusGeometry browser](https://threejs.org/docs/scenes/geometry-browser.html#TorusGeometry) reference):** **`still-scene`** uses geometry-browser **`Material`** palette; **`animated-scene`** still uses legacy approximate colors until Material milestone completes; **`DirectionalLight::intensity`** shipped (**export bins deliberately at **`1.0`**, not browser **`3`** on one light — **`doc/notes-on-lighting.md`**); black clear only (reference **`0x444444`**); single light (reference: **three** directionals at **`intensity` 3**, summed in **Multi-light**). **Out of Phase 2 scope (by choice):** wireframe overlay, flat shading, extended torus API (**`radius`**, **`tube`**, **`arc`**), live **`winit`** viewer / GUI.
...
1. **Phase 2 — material (single light)** — explicit **`Material`** on **`Shape`** (diffuse **`0x156289`**, emissive **`0x072534`**, specular **`0x111111`**, **`shininess` 30**); **`DirectionalLight`** **`intensity`** (white only; **`new`** → **`1.0`** in export bins); **`FrameBuffer::clear(Rgb(68, 68, 68))`**; export bins with **one** light at **`intensity` 1.0**.
2. **Phase 2 — multi-light** — **`Shape::render(&[DirectionalLight])`** summation; three white directionals at **`intensity` 3** each in export bins (geometry-browser positions); first milestone that raises bin intensity above **`1.0`**.
